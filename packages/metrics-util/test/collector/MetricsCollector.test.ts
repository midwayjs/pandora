...
import {NormalMetricsCollector} from '../../src/collector/NormalMetricsCollector';
import {MetricsCollector} from '../../src/collector/MetricsCollector';
import {CompactMetricsCollector} from '../../src/collector/CompactMetricsCollector';
...
describe('/test/unit/collector/MetricsCollector.test.ts', () => {
  ...
  it('should create new NormalMetricsCollector', async () => {
    let timestamp = Date.now();
    let collector = new NormalMetricsCollector({globalTags: {}, rateFactor: 10, durationFactor: 10, reportInterval: 60});
    collector.collectGauge(MetricName.build('collector.gauge'), 10, timestamp);
  ...
  it('should test getNormalizedStartTime', function () {
    let collector = new MetricsCollector({globalTags: {}, rateFactor: 10, durationFactor: 1, reportInterval: 60});
