...
import {MetricObject, MetricType} from './MetricObject';
...
export class MetricsCollector {
  ...
  addMetricWithSuffix(name: MetricName, suffix: string, value: any, timestamp: number, type: MetricType = MetricType.GAUGE, interval: number = -1) {
    return this.addMetric(name.resolve(suffix), value, timestamp, type, interval);
  }

  addMetric(fullName: MetricName, value: any, timestamp: number, type: MetricType = MetricType.GAUGE, interval: number = -1) {
