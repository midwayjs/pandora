import {
  ...
  componentConfig,
  dependencies,
} from '../src/ComponentReflector';
import { expect } from 'chai';
describe('ComponentReflector', () => {
  it('should decorating componentName be ok', () => {
    const expectName = 'expectName';
    @componentName(expectName)
    class TestClass {}
    expect(ComponentReflector.getComponentName(TestClass)).to.be.equal(
      expectName
    );
  });
  it('should decorating componentConfig be ok', () => {
    ...
    class TestClass {}
    expect(ComponentReflector.getComponentConfig(TestClass)).to.be.equal(
      expectConfig
    );
  });
  it('should decorating dependencies be ok', () => {
    ...
    class TestClass {}
    expect(ComponentReflector.getDependencies(TestClass)).to.be.equal(
      expectDependencies
    );
  });
});
