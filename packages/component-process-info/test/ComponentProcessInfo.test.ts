...
describe('ComponentProcessInfo', () => {
  ...
  it('should have correct meta info', () => {
    expect(
      ComponentReflector.getComponentName(ComponentProcessInfo)
    ).to.be.equal('processInfo');
    expect(
      ComponentReflector.getDependencies(ComponentProcessInfo)
    ).to.deep.equal(['actuatorServer', 'indicator']);
  });

  it('should start be ok', async () => {
    expect(ctx.indicatorManager.get('process')[0]).be.an.instanceof(
      ProcessInfoIndicator
    );
  });
