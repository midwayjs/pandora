...
export class IndicatorManager {
  ...
  async invokeRaw(group: string, query: any = {}): Promise<IndicatorResultObject[]> {
    ...
    for(const indicator of indicators) {
      ...
      if((query.key && key !== query.key) || (query.scope && scope !== query.scope)) {
        continue;
      }
      const data = await indicator.invoke(query);
      ret.push({
        group, key, data, scope,
        pid: PID, appName: this.ctx.appName});
    }
    return ret;
  }
  ...
  async invokeAllProcessesRaw(group: string, query: any = {}): Promise<IndicatorResultObject[]> {
    const selectors = {
      appName: query.appName,
      pid: query.pid,
    };
    const hubFacade: HubFacade = this.ctx.hubFacade;
    const consumer = hubFacade.getConsumer({name: IndicatorManagerProxy.SERVICE_NAME_AT_IPC_HUB});
  ...
  async publish() {
    const hubFacade: HubFacade = this.ctx.hubFacade;
    await hubFacade.publish(this.proxy, {name: IndicatorManagerProxy.SERVICE_NAME_AT_IPC_HUB});
  }
