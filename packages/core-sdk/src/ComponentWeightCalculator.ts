...
export class ComponentWeightCalculator {
  ...
  getWeight(name, chain?: string[]) {
    ...
    assert(
      -1 === chain.indexOf(name),
      `Component name: ${name} in a cyclic dependency chain: ${chain.join(
        ' -> '
      )} -> ${name}`
    );
    if (chain.length > 1 && name === 'all') {
      throw new Error(
        `Reserved component name 'all' not allowed to contains within a dependency chain: ${chain.join(
          ' -> '
        )} -> ${name}`
      );
    }
    ...
    assert(
      ...
      `Could not found component, name: ${name}`
    );
    const componentDeclaration = this.components.get(name);
    if (
      !componentDeclaration.dependencies ||
