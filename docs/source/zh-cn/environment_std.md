...
最基本的 `Environment` 对象是约束于 Interface [Environment](classes/Environment.html) 。
...
详细参见类型信息： [BaseEnvironment](classes/BaseEnvironment.html) 
...
如果用户不注入自定义的 `Environment` 类，我们将使用默认的 `DefaultEnvironment` 。
